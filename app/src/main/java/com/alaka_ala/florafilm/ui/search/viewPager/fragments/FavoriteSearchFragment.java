...
import com.alaka_ala.florafilm.databinding.FragmentFavoriteSearchBinding;
...
public class FavoriteSearchFragment extends Fragment implements SearchFragment.SearchListenerResult {
    private FragmentFavoriteSearchBinding binding;
    private RecyclerView rvFavoriteSearch;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rvFavoriteSearch = binding.rvFavoriteSearch;
        rvFavoriteSearch.setLayoutManager(new GridLayoutManager(getContext(), 3, RecyclerView.VERTICAL, false));
        utils = new UtilsFavoriteAndViewFilm(getContext());
        SearchFragment.setSearchListenerResult(this);

        rvFavoriteSearch.addOnItemTouchListener(new MyRecyclerViewItemTouchListener(getContext(), rvFavoriteSearch, new MyRecyclerViewItemTouchListener.OnItemClickListener() {
            @Override
    ...
    public void result(String query, Collection collection) {
        ...
        if (this.collection.getItems().isEmpty()) {
            rvFavoriteSearch.setLayoutManager(new GridLayoutManager(getContext(), 1, RecyclerView.VERTICAL, false));
            rvFavoriteSearch.setAdapter(adapter = new UniversalRecyclerAdapter(rvFavoriteSearch, getLayoutInflater(), utils, collection, UniversalRecyclerAdapter.TYPE_NULL_DATA));
            return;
        }
        rvFavoriteSearch.setLayoutManager(new GridLayoutManager(getContext(), 3, RecyclerView.VERTICAL, false));
        rvFavoriteSearch.setAdapter(adapter = new UniversalRecyclerAdapter(rvFavoriteSearch, getLayoutInflater(), utils, collection, UniversalRecyclerAdapter.TYPE_FULL_WIDTH));
    }
